...
const TreinamentoCard = ({ treinamento, onAction }) => {
  return (
    <motion.div
      whileHover={{ y: -5 }}
    ...
    >
      <Card className="h-full flex flex-col overflow-hidden card-hover">
        ...
        <CardHeader>
          ...
          <CardDescription className="h-12 overflow-hidden text-ellipsis">{treinamento.descricao}</CardDescription>
        </CardHeader>
        <CardContent className="flex-grow">
          <div className="space-y-4">
            <div>
              ...
              <Progress value={treinamento.progresso} />
            </div>
            <div className="flex justify-between text-sm text-muted-foreground">
              <div className="flex items-center">
                <FileText className="mr-1 h-4 w-4" />
                <span>{treinamento.modulosConcluidos}/{treinamento.modulos} módulos</span>
              </div>
              <div className="flex items-center">
                <Users className="mr-1 h-4 w-4" />
                <span>{treinamento.cargo}</span>
              </div>
            </div>
          </div>
        </CardContent>
        <CardFooter>
          {treinamento.progresso === 100 ? (
            <Button className="w-full" variant="outline">
              <CheckCircle className="mr-2 h-4 w-4" />
              Concluído
            </Button>
          ) : treinamento.progresso > 0 ? (
            <Button className="w-full" onClick={() => onAction(treinamento.id, "continuar")}>
              <Play className="mr-2 h-4 w-4" />
              Continuar
            </Button>
          ) : (
            <Button className="w-full" variant="outline" onClick={() => onAction(treinamento.id, "iniciar")}>
              <Play className="mr-2 h-4 w-4" />
              Iniciar
            </Button>
          )}
